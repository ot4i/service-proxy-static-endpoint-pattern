...
com.ibm.etools.mft.pattern.sv.sp.ssp.PatternMessages messages = new com.ibm.etools.mft.pattern.sv.sp.ssp.PatternMessages();
        ...
        out.write(NL);         
        out.write(NL);         
        out.write("CREATE COMPUTE MODULE Build_SOAP_Fault");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tCREATE FUNCTION Main() RETURNS BOOLEAN");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tBEGIN");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t/* An unspecified error has occurred before successful completion of service call.");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t   Return a fault to the requester indicating an error in the broker intermediary.");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t** Note that modelled faults will be returned directly to the requester. This is reasonable as both requester and provider should operate on the same interface");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t*/");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tDECLARE soapenv NAMESPACE 'http://schemas.xmlsoap.org/soap/envelope/';");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tSET OutputRoot.SOAP.Context.Namespace.(SOAP.NamespaceDecl)xmlns:soapenv = 'http://schemas.xmlsoap.org/soap/envelope/';");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tSET OutputRoot.SOAP.Body.soapenv:Fault.faultcode = 'soapenv:Server';\t");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tSET OutputRoot.SOAP.Body.soapenv:Fault.faultstring = '");  //$NON-NLS-1$        
        out.write( pattern.getString("com.ibm.etools.mft.pattern.sv.sp.ssp.esql.1") );
        out.write("';");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tSET OutputRoot.SOAP.Body.soapenv:Fault.detail.Message = '");  //$NON-NLS-1$        
        out.write( pattern.getString("com.ibm.etools.mft.pattern.sv.sp.ssp.esql.2") );
        out.write("';");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tSET OutputLocalEnvironment.Destination.SOAP.Reply.Transport.HTTP.ReplyStatusCode = 500;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\t");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\t-- Add some exception data - error and text only");  //$NON-NLS-1$        
        out.write(NL);         
        ...
        out.write(NL);         
        out.write("\t\tDECLARE numSubs INTEGER;");  //$NON-NLS-1$        
        out.write(NL);         
        ...
        out.write(NL);         
        out.write("\t\t\t\tSET OutputRoot.SOAP.Body.soapenv:Fault.detail.Exceptions[I].Error = Error;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\t\t\tSET OutputRoot.SOAP.Body.soapenv:Fault.detail.Exceptions[I].Text = Text;");  //$NON-NLS-1$        
        out.write(NL);         
        ...
        out.write(NL);         
        out.write("    SET OutRef.Service.name = InputRoot.SOAP.Context.service;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tSET OutRef.Service.nameSpace = InputRoot.SOAP.Context.Namespace;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tSET OutRef.Service.port = InputRoot.SOAP.Context.port;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tSET OutRef.Service.portType = InputRoot.SOAP.Context.portType;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tSET OutRef.Service.operation = InputRoot.SOAP.Context.operation;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tSET OutRef.Service.targetEndpoint = Environment.PatternVariables.TargetEndpoint;");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t");  //$NON-NLS-1$        
        out.write(NL);         
        ...
        out.write( pattern.getString("com.ibm.etools.mft.pattern.sv.sp.ssp.esql.4") );
        out.write("';");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\tDECLARE I INTEGER 1;");  //$NON-NLS-1$        
        out.write(NL);         
        ...
        out.write( pattern.getString("com.ibm.etools.mft.pattern.sv.sp.ssp.esql.5") );
        out.write(" ' || COALESCE(Label, ''); ");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\t\t\tSET FaultText = FaultText || ' ");  //$NON-NLS-1$        
        out.write( pattern.getString("com.ibm.etools.mft.pattern.sv.sp.ssp.esql.6") );
        out.write(" ' || COALESCE(CAST(Error AS CHARACTER), '');");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\t\t\tSET FaultText = FaultText || ' ");  //$NON-NLS-1$        
        out.write( pattern.getString("com.ibm.etools.mft.pattern.sv.sp.ssp.esql.7") );
        out.write(" ' || COALESCE(Text, '');");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\t\t\t\tSET FaultText = FaultText || ' ");  //$NON-NLS-1$        
        out.write( pattern.getString("com.ibm.etools.mft.pattern.sv.sp.ssp.esql.8") );
        ...
        out.write(NL);         
        out.write(NL);         
        out.write("CREATE FILTER MODULE CheckErrorLoggingOn");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tCREATE FUNCTION Main() RETURNS BOOLEAN");  //$NON-NLS-1$        
        out.write(NL);         
        out.write("\tBEGIN");  //$NON-NLS-1$        
